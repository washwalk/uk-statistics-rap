...
def source_points(payload: dict) -> tuple[str, list[dict]]:
    ...
    raise ValueError("Source payload did not contain years, quarters, or months")
...
def transform() -> None:
    ...
    for point in points:
        value_text = str(point.get("value", "")).replace(",", "").strip()
        if not value_text:
            continue
        value = float(value_text)
        change = "" if previous_value is None else int(round(value - previous_value))
        percent_change = "" if previous_value in (None, 0) else round(((value / previous_value) - 1) * 100, 3)
        rows.append(
            {
