...
def source_points(payload: dict) -> tuple[str, list[dict]]:
    for period_type in ("quarters", "months", "years"):
        points = payload.get(period_type) or []
        if points:
            return period_type, points
    raise ValueError("Source payload did not contain quarters, months, or years")
...
def build_rows(config: dict, payload: dict) -> list[dict]:
    ...
    for point in points:
        ...
        growth = "" if previous_value in (None, 0) else round(((value / previous_value) - 1) * 100, 3)
        rows.append(
            {
                ...
                "value": value,
                "growth_from_previous_percent": growth,
